...
async function pairWithProcessor(host, displayName = 'Savant Bridge') {
  console.log('[pairing] Generating key pair...');
  const { privateKey, csr } = await generateKeyAndCSR(displayName);

  console.log(`[pairing] Connecting to ${host}:${PAIRING_PORT}...`);
  ...
  return new Promise((resolve, reject) => {
    ...
    function sendCSR() {
      ...
      console.log(`[pairing] → sending CSR (${payload.length} bytes)`);
      socket.write(payload);
    }

    socket.on('secureConnect', () => {
      console.log('[pairing] TLS connected — press the pairing button on the QSX now...');
    });
    ...
    function processMessage(line) {
      if (!line.trim()) return;
      console.log(`[pairing] ← ${line.slice(0, 500)}`);
      ...
      if (!clientTag && contentType.startsWith('status')) {
        ...
        console.log(`[pairing] Server status received — permissions: ${JSON.stringify(permissions)}`);
        if (permissions.includes('PhysicalAccess')) {
          console.log('[pairing] PhysicalAccess granted — sending CSR...');
          sendCSR();
        } else {
          console.log('[pairing] Waiting for button press on QSX...');
        }
        return;
      }
      ...
      console.log(`[pairing] Received message (tag=${clientTag}, type=${contentType})`);
    }
